...
def ensure_baseline_hidden(act_dict, n_iti_pre=3):
    """Return act_dict with ``baseline_hidden`` reconstructed when it is missing.

    Older inference dumps (and any vis_data.pkl predating the baseline-capture
    change) store only the single-stream ``hidden_states`` (T, H) plus the
    per-trial ``trial_structure`` (with ``stim_window``). The responder t-test
    needs a pre-stimulus baseline window per trial, so here we rebuild it as the
    ``n_iti_pre`` timesteps immediately before each trial's stim onset (padding
    with zeros if the ITI is shorter). No-op if ``baseline_hidden`` already
    exists, if the stream/structure aren't available, or if the reconstructed
    trial count doesn't match an existing ``stim_hidden`` (so trials never get
    mis-paired). Returns a shallow copy when it adds the key.
    """
    if "baseline_hidden" in act_dict:
        return act_dict
    hs = act_dict.get("hidden_states")
    ts = act_dict.get("trial_structure")
    if hs is None or ts is None:
        return act_dict
    hs = np.asarray(hs)
    if hs.ndim != 2:                 # need a single (T, H) stream to slice into
        return act_dict
    T, H = hs.shape
    base = []
    for tr in ts:
        ss = int(tr["stim_window"][0])
        bs0 = ss - n_iti_pre
        if bs0 >= 0:
            base.append(hs[bs0:ss])
        else:
            pad = np.zeros((n_iti_pre, H), dtype=hs.dtype)
            if ss > 0:
                pad[-ss:] = hs[:ss]
            base.append(pad)
    base = np.stack(base)            # (n_trials, n_iti_pre, H)
    sh = act_dict.get("stim_hidden")
    if sh is not None and np.asarray(sh).shape[0] != base.shape[0]:
        return act_dict              # trial counts disagree — don't risk mis-pairing
    out = dict(act_dict)
    out["baseline_hidden"] = base
    return out


def responsive_proportions_ttest(act_dict, period="stim", alpha=0.05,
                                  n_sub=1000, n_rep=1, direction="excitatory",
                                  min_cohens_d=0.0, min_resp_frac=0.0,
                                  random_state=0):
    """Fraction of units significantly responsive to each stimulus vs baseline.

    Matches the experimental selectivity convention: for each unit and each
    stimulus, a paired t-test of the per-trial response during ``period`` against
    that trial's pre-stimulus baseline (requires ``act_dict['baseline_hidden']``).
    Unlike :func:`preferred_stim_proportions` this is NOT winner-take-all — a unit
    can be responsive to several stimuli (mixed selectivity), exactly as a neuron
    can be significantly driven by more than one stimulus.

    Default behaviour (``n_rep=1``): a single deterministic paired t-test on the
    first ``n_sub`` trials per stimulus (or all of them if fewer / ``n_sub=None``).
    Fixing ``n_sub`` keeps the statistical power identical across models so the
    proportions are directly comparable. Empirically the proportions are robust to
    ``n_sub`` for these networks (unit effect sizes are bimodal), so no resampling
    is needed; set ``n_rep>1`` to fall back to the random-subsample majority-vote
    scheme (useful only if your effects are not bimodal).

    Args:
        act_dict: activations dict with 'stim_hidden'/'reward_hidden',
            'baseline_hidden' (n_trials, n_base_t, n_units) and 'stimulus'.
        period: "stim" or "reward".
        alpha: significance level (one-sided when direction != "two-sided").
        n_sub: trials per stimulus to use (None = all). Fixed for comparability.
        n_rep: 1 = single deterministic test (default); >1 = random subsamples +
            majority vote.
        direction: "excitatory" (response>baseline), "suppressed", or "two-sided".
        min_cohens_d: if >0, also require |Cohen's d| = |mean/std| >= this. NOTE
            this normalises out magnitude (a tiny-but-consistent response still has
            large d), so it does little to grade the mid-stim count; prefer
            min_resp_frac for that. 0 = off (default).
        min_resp_frac: if >0, also require the unit's mean response amplitude to a
            stimulus to be at least this fraction of that unit's PEAK mean response
            across stimuli (a scale-free amplitude / fraction-of-dynamic-range
            criterion, like an experimental amplitude threshold). Because mid-stim
            responses are a small fraction of each unit's peak, this gates out weak
            mid responders and pulls the mid responsive fraction toward halfway
            (~0.1-0.15 centers it for these networks). 0 = off (default).
        random_state: RNG seed (only used when n_rep > 1).

    Returns:
        dict with
            frac_per_stim (n_stim,) fraction of units responsive to each stimulus,
            n_responsive  (n_stim,) counts,
            n_units       int,
            responsive    (n_units, n_stim) bool responsiveness,
            resp_prob     (n_units, n_stim) 0/1 (single test) or fraction of
                          repeats significant (majority-vote mode).
    """
    resp_key = "stim_hidden" if period == "stim" else "reward_hidden"
    R = np.asarray(act_dict[resp_key]).mean(axis=1)            # (n_trials, units)
    B = np.asarray(act_dict["baseline_hidden"]).mean(axis=1)   # (n_trials, units)
    delta = R - B                                              # paired difference
    stim = np.asarray(act_dict["stimulus"])
    n_units = delta.shape[1]
    n_stim = int(stim.max()) + 1
    rng = np.random.default_rng(random_state)

    def _sig(d):
        t, p = ttest_1samp(d, 0.0, axis=0)
        if direction == "excitatory":
            s_ = (t > 0) & (p / 2 < alpha)
            ...
            s_ = p < alpha
        if min_cohens_d > 0.0:
            # Cohen's d for a one-sample test = mean/std = t / sqrt(n). Gate on
            # effect SIZE, not just reliability: drops weak (e.g. mid-stim)
            # responders so the responsive COUNT tracks response magnitude, which
            # is graded by value (matches an amplitude criterion in experiments).
            cohens_d = np.asarray(t, dtype=float) / np.sqrt(d.shape[0])
            s_ = np.asarray(s_) & (np.abs(cohens_d) >= min_cohens_d)
        return np.nan_to_num(np.asarray(s_, dtype=float))

    resp_prob = np.zeros((n_units, n_stim))
    amp = np.zeros((n_units, n_stim))                          # mean response amplitude
    for s in range(n_stim):
        idx = np.where(stim == s)[0]
        if len(idx) == 0:
            continue
        if n_rep <= 1:
            take = idx if n_sub is None else idx[:min(int(n_sub), len(idx))]
            resp_prob[:, s] = _sig(delta[take])                # 0/1 single test
            amp[:, s] = delta[take].mean(axis=0)
        else:
            hits = np.zeros(n_units)
            for _ in range(n_rep):
                k = len(idx) if n_sub is None else min(int(n_sub), len(idx))
                take = rng.choice(idx, size=k, replace=(n_sub is not None and len(idx) < n_sub))
                hits += _sig(delta[take])
            resp_prob[:, s] = hits / n_rep
            amp[:, s] = delta[idx].mean(axis=0)

    responsive = resp_prob >= 0.5
    if min_resp_frac > 0.0:
        # amplitude must be >= min_resp_frac of the unit's strongest stim response
        peak = np.clip(amp.max(axis=1, keepdims=True), 1e-9, None)
        responsive = responsive & (amp >= min_resp_frac * peak)
    return dict(
        frac_per_stim=responsive.mean(axis=0),
        n_responsive=responsive.sum(axis=0).astype(int),
        n_units=n_units,
...
# ── gauge-invariant low-rank connectivity interpretation ───────────────────
#
# A LowRankRNN's recurrent weight is W_hh = M @ N.T / H. Only this PRODUCT
# matters for the network's behaviour: M -> M@A, N -> N@A^-T for ANY
# invertible (rank x rank) A leaves W_hh (and the whole network) unchanged.
# So raw M, N entries -- and quantities derived from them individually, like
# per-unit loading-vector norms -- are NOT comparable across separately
# trained models (different seeds/ranks land on different, arbitrary A).
#
# What IS well-defined and gauge-invariant:
#   (a) any function of W_hh itself (it's the same matrix regardless of which
#       (M, N) factorisation produced it), including its entries and its SVD
#       (singular vectors are canonical up to sign/degenerate rotation);
#   (b) anything computed purely from recorded hidden activity (tuning,
#       selectivity, decoding, RDMs) -- see the rest of this module.
# This gives a route to genuinely comparable "modes" across ranks/seeds
# WITHOUT changing the architecture: reconstruct W_hh, take its SVD, and
# relate the canonical singular vectors to input/readout weights and
# empirical tuning (the Dubreuil/Mastrogiuseppe/Ostojic population-structure
# approach).

def lowrank_weight_matrix(backbone):
    """Reconstruct the effective (H, H) recurrent weight matrix of a
    LowRankRNN: W_hh = M @ N.T / H (+ J0 if the model has a fixed random
    full-rank component). This full matrix is gauge-invariant even though the
    (M, N) factorisation that produced it is not.

    Args:
        backbone: a cxval.models.LowRankRNN (or LowRankLeakyRNN) instance.

    Returns:
        (H, H) numpy array.
    """
    M = backbone.m.detach().cpu().numpy()
    N = backbone.n.detach().cpu().numpy()
    W = (M @ N.T) / backbone.hidden_size
    if getattr(backbone, "J0", None) is not None:
        W = W + backbone.J0.detach().cpu().numpy().T
    return W


def lowrank_svd_modes(backbone, n_modes=None):
    """Canonical (gauge-free) low-rank modes via SVD of the effective W_hh.

    Because W_hh = M @ N.T / H is itself gauge-invariant, its singular
    vectors are a canonical basis for the rank-R connectivity -- unlike the
    raw (m_i, n_i) loading vectors, U/V here are directly comparable across
    models of the same rank (up to an overall sign per mode, and up to
    rotation within a degenerate singular-value subspace).

    Args:
        backbone: a LowRankRNN instance.
        n_modes: number of singular modes to keep (default: backbone.rank).

    Returns:
        U: (H, n_modes) left singular vectors ("output"/write directions,
           analogous to M but canonical).
        S: (n_modes,) singular values (mode strength).
        V: (H, n_modes) right singular vectors ("input"/read directions,
           analogous to N but canonical).
    """
    W = lowrank_weight_matrix(backbone)
    U, S, Vt = np.linalg.svd(W, full_matrices=False)
    R = n_modes or getattr(backbone, "rank", U.shape[1])
    return U[:, :R], S[:R], Vt[:R].T


def lowrank_mode_alignment(model, backbone_attr="backbone", n_modes=None):
    """Correlate each canonical low-rank mode with the model's input and
    readout weights -- a gauge-invariant answer to "what does mode r mean?".

    For each of the R singular modes (from lowrank_svd_modes), computes the
    cosine similarity between:
      - the mode's READ direction (V column) and each input channel's weight
        column (input2h.weight[:, k], one per context/stimulus/reward-cue
        input dimension) -- "how much does input channel k drive mode r?"
      - the mode's WRITE direction (U column) and the model's readout weights
        (vigour_head / value_head, restricted to the readout units) -- "how
        much does mode r drive vigour/value output?"

    Args:
        model: a VigourActorCritic (or ActorCritic) with a LowRankRNN
            backbone at ``getattr(model, backbone_attr)``.
        n_modes: number of modes (default: backbone.rank).

    Returns:
        dict with:
          S              (R,) singular values (mode strength).
          input_cosine   (R, input_size) cosine sim of each mode's read
                         direction with each input channel's weight column.
          vigour_cosine  (R,) cosine sim of each mode's write direction with
                         the vigour-head readout weight (zero-padded outside
                         the readout units).
          value_cosine   (R,) same, for the value-head readout weight.
    """
    backbone = getattr(model, backbone_attr)
    U, S, V = lowrank_svd_modes(backbone, n_modes=n_modes)
    R = U.shape[1]
    ...
    W_in = backbone.input2h.weight.detach().cpu().numpy()   # (H, input_size)
    input_cosine = np.array([[_cos(V[:, r], W_in[:, k]) for k in range(W_in.shape[1])]
                             for r in range(R)])

    H = backbone.hidden_size
    n_readout = getattr(model, "n_readout", H)
    vigour_cosine = np.full(R, np.nan)
    value_cosine = np.full(R, np.nan)
    if hasattr(model, "vigour_head"):
        w_vig = np.zeros(H, dtype=np.float32)
        w_vig[:n_readout] = model.vigour_head.weight.detach().cpu().numpy().flatten()
        vigour_cosine = np.array([_cos(U[:, r], w_vig) for r in range(R)])
    if hasattr(model, "value_head"):
        w_val = np.zeros(H, dtype=np.float32)
        w_val[:n_readout] = model.value_head.weight.detach().cpu().numpy().flatten()
        value_cosine = np.array([_cos(U[:, r], w_val) for r in range(R)])

    return dict(S=S, input_cosine=input_cosine, vigour_cosine=vigour_cosine,
                value_cosine=value_cosine)


def lowrank_unit_tuning_alignment(backbone, activations, period="stim"):
    """Per-unit correlation between empirical stimulus tuning and the unit's
    row/column norms in the effective W_hh -- both sides of this correlation
    are gauge-invariant (empirical tuning is computed from actual hidden
    activity; W_hh row/column norms are properties of the fixed, gauge-free
    connectivity matrix), so this is directly comparable across ranks/seeds.

    Args:
        backbone: a LowRankRNN instance.
        activations: activations dict (stim_hidden/reward_hidden, stimulus,
            context) as produced by infer_context_vigour / infer_vigour.
        period: "stim" or "reward".

    Returns:
        dict with:
          selectivity   (H,) selectivity_index_range per unit (empirical).
          row_norm      (H,) ||W_hh[i, :]|| -- how strongly unit i's activity
                        drives the rest of the recurrent population.
          col_norm      (H,) ||W_hh[:, i]|| -- how strongly unit i is driven
                        by the low-rank recurrent structure.
          corr_row      Pearson r between selectivity and row_norm.
          corr_col      Pearson r between selectivity and col_norm.
    """
    W = lowrank_weight_matrix(backbone)
    ...
    si = selectivity_index_range(tuning)
    ...
    return dict(selectivity=si, row_norm=row_norm, col_norm=col_norm,
                corr_row=_safe_corr(si, row_norm), corr_col=_safe_corr(si, col_norm))


# ── training-outcome classification (context-switch vigour task) ──────────
#
# Deliberately pure numpy (no torch) so it can be imported by lightweight,
# torch-free analysis/plotting scripts as well as the training pipeline
# itself. Operates on the COLUMNAR per-trial dicts produced by
# cxval.context_vigour.build_context_trial_data (keys: stimulus, context,
# vigour, trial_in_episode, ...), not on the hidden-activation dicts the rest
# of this module works with.

def classify_outcome(trial_data, value_matrix, frac=0.2,
                     collapse_thresh=0.05, converged_thresh=0.8,
                     context_only_thresh=0.3):
    """Classify a run's vigour behaviour from its per-trial record.

    Distinguishes two failure modes that both look like "near-zero value
    discrimination" if you only look at the value/context gap, but are very
    different: a genuine zero-vigour policy collapse (mean vigour ~0
    everywhere -- the agent stopped licking at all) vs. a nonzero but
    value-undiscriminating response (e.g. context-only: vigour tracks which
    context it's in but not which stimulus, as seen at short training
    budgets on the 4-stimulus/2-context swap task).

    Args:
        trial_data: columnar per-trial dict (stimulus, context, vigour,
            trial_in_episode, ...) -- either the full training-rollout record
            or a held-out inference record.
        value_matrix: (n_stim, n_ctx) true reward-probability array.
        frac: fraction of trials (by trial_in_episode, i.e. the END of the
            sequence) to use. Use a small fraction (e.g. 0.2) for a TRAINING
            rollout record, to characterise where training ended up rather
            than averaging over the whole learning trajectory. Use None (all
            trials) for an already-deterministic held-out inference record,
            where there's no "still learning" transient to exclude.
        collapse_thresh: mean vigour across ALL conditions below this =
            "collapsed" (policy stuck emitting ~0 regardless of stimulus/
            context).
        converged_thresh: value_gap (mean vigour at true value=1 minus at
            true value=0, pooled across contexts) above this = "converged".
        context_only_thresh: value_gap below this (and NOT collapsed) =
            "context_only" (nonzero vigour, but not discriminating by value).
            Between context_only_thresh and converged_thresh = "partial".

    Returns:
        dict with outcome (one of "collapsed"/"context_only"/"partial"/
        "converged"/"no_data"), late_mean_vigour, late_value_gap.
    """
    value_matrix = np.asarray(value_matrix, dtype=np.float32)
    vig = np.asarray(trial_data["vigour"])
    if vig.size == 0:
        return dict(outcome="no_data", late_mean_vigour=np.nan, late_value_gap=np.nan)

    if frac is not None:
        x = np.asarray(trial_data["trial_in_episode"])
        mask = x > (1.0 - frac) * x.max()
    else:
        mask = np.ones_like(vig, dtype=bool)

    stim = np.asarray(trial_data["stimulus"])[mask]
    ctx = np.asarray(trial_data["context"])[mask]
    v = vig[mask]
    if v.size == 0:
        return dict(outcome="no_data", late_mean_vigour=np.nan, late_value_gap=np.nan)

    true_val = value_matrix[stim, ctx]
    mean_vig = float(v.mean())
    # Use value_matrix's own actual high/low levels, not hardcoded ~1.0/~0.0
    # -- with a PROBABILISTIC value_matrix (e.g. p_low=0.2/p_high=0.8, see
    # build_swap_value_matrix), true_val never reaches 0.999/0.001, so the
    # old hardcoded thresholds matched nothing and always gave "no_data".
    eps = 1e-3
    p_hi, p_lo = float(value_matrix.max()), float(value_matrix.min())
    hi = v[true_val >= p_hi - eps]
    lo = v[true_val <= p_lo + eps]
    gap = float(hi.mean() - lo.mean()) if len(hi) and len(lo) else np.nan

    if mean_vig < collapse_thresh:
        outcome = "collapsed"
    elif np.isnan(gap):
        outcome = "no_data"
    elif gap >= converged_thresh:
        outcome = "converged"
    elif gap < context_only_thresh:
        outcome = "context_only"
    else:
        outcome = "partial"

    return dict(outcome=outcome, late_mean_vigour=mean_vig, late_value_gap=gap)
