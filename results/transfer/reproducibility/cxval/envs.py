...
class TaskEnv:
    ...
    Lick during reward window, reward not available → reward_lick_fa  (default -1)
    ...
    def __init__(
        ...
        reward_lick_fa=-1.0,
        lick_cost=0.0,
        reward_lick_miss=None,   # old name — accepted for backward compat with saved vis_data
    ):
        if reward_lick_miss is not None and reward_lick_fa == -1.0:
            reward_lick_fa = reward_lick_miss
        ...
        self.reward_lick_fa = reward_lick_fa
    ...
    def step(self, action):
        ...
        if in_window and licked and not self._licked_this_window:
            ...
            reward = self.reward_lick if reward_available else self.reward_lick_fa
        else:
