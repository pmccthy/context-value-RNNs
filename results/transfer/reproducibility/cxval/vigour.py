...
class VigourActorCritic(nn.Module):
    ...
    def __init__(self, backbone: RNN, action_std=0.1, readout_fraction=0.5,
                 demean_readout=False, readout_mode=None, aux_n_stim=0):
        super().__init__()
    ...
    def squash(self, mean):
        """Map the head output to a vigour in [0,1]: hard clamp for the score
        (Gaussian) policy, smooth sigmoid for the pathwise (deterministic) policy."""
        return torch.sigmoid(mean) if self.policy_mode == "pathwise" else mean.clamp(0.0, 1.0)
...
def train_vigour(value_matrix, *, batch_size=32, n_trials_per_episode=1500,
                 ...
                 nonneg_coef=0.0, detach_readout=False,
    ...
    while not done:
        ...
        if need_act_buf:
            ...
            if need_iti_mask:
                actmask_buf.append(iti_mask_bt[:, gt] if gt < T
                                   else torch.zeros(B, device=device))
        if ac.stim_head is not None and gt < T:
            aux_buf.append((ac.stim_head(hidden), stim_id_bt[:, gt], aux_mask_bt[:, gt]))
        if policy == "pathwise":
            v = torch.sigmoid(mean)                   # smooth squash → gradient always flows
            b_t = base_bt[:, gt] if gt < T else torch.zeros_like(mean)
            a_t = active_bt[:, gt] if gt < T else torch.zeros_like(mean)
        ...
        if t_win % bptt_len == 0 or done:
            ...
            if checkpoint_dir is not None:
                u = len(grad_norms)
                # Fine-grained window at the START of this train_vigour() call
                # (for train_reversal.py, that's immediately after the
                # reversal begins -- exactly where the "sudden change" is) --
                # checkpoint every checkpoint_fine_every updates through
                # checkpoint_fine_until, then fall back to the regular
                # checkpoint_every grid. u == 1 is always included so the very
                # first update is never missed even if fine sampling is off.
                if checkpoint_fine_until > 0 and u <= checkpoint_fine_until:
                    do_save = (u == 1) or (u % checkpoint_fine_every == 0)
                else:
                    do_save = (u == 1) or (u % checkpoint_every == 0)
                if do_save:
                    torch.save({k: v.cpu().clone() for k, v in ac.state_dict().items()},
                               checkpoint_dir / f"checkpoint_{u:05d}.pt")
            lp_buf, val_buf, rew_buf, msk_buf, aux_buf, pw_buf, act_buf, actmask_buf = \
                [], [], [], [], [], [], [], []
            t_win = 0
...
def infer_value(model, value_matrix, *, n_eval_episodes=12, n_trials_per_episode=400,
               base_seed=10_000, device="cpu", vigour_cost=1.1, cost_type="quadratic",
               reward_fa=0.0, stim_timesteps=5, reward_timesteps=3, iti_timesteps=(3, 8)):
    """Deterministic eval: mean CRITIC value estimate V(s) per stimulus -- added
    to this reproducibility-pinned cxval snapshot (which predates infer_value
    in the live cxval/vigour.py) so make_probe() can log a genuine trial-
    resolved value curve alongside probe_vigour, without touching any
    existing function's behaviour. Same rollout structure as infer_vigour
    immediately above -- the critic value comes from the SAME model.step()
    call infer_vigour already makes (it just discards it as `_`), so this
    costs one extra rollout, same cost class as infer_vigour/infer_rpe in
    the live library.
    """
    device = torch.device(device)
    value_matrix = np.asarray(value_matrix, dtype=np.float32)
