...
# The style sheet is named "16_06_26_figure_style.mplstyle" next to this file in
# scripts/, but "figure_style.mplstyle" (no prefix) in every bundle this file gets
# copied into (code/figure_style.mplstyle). Resolving both means a plain `cp` of this
# file into a bundle can't silently break styling by pointing at a filename that isn't
# there — that exact failure mode happened once already (main() below only applies the
# style file if it exists, with no warning if it doesn't).
DEFAULT_STYLE = next((p for p in (Path(__file__).with_name("figure_style.mplstyle"),
                                  Path(__file__).with_name("16_06_26_figure_style.mplstyle"))
                     if p.exists()), Path(__file__).with_name("figure_style.mplstyle"))
...
def heatmap_montage(D, model_type, *, vmax=None, ncol=6, title=None):
    """Tile heatmap_seed across all of a model's seeds; returns the Figure (or None).
    Shared colour scale (vmax) across panels. Pass `title` to override the default
    suptitle (e.g. to label which seed subset this montage is — the default text only
    names the model, not e.g. which outcome group the seeds were filtered to)."""
    ti = D["model_types"].index(model_type)
    ...
    # a FIXED amount of vertical room is reserved for the suptitle regardless of nrow —
    # with few seeds (small nrow) a height scaled only by nrow is too short and the
    # suptitle collides with the panel titles / colorbar; this keeps it clear at any size.
    top_pad = 0.9
    fig_h = 1.7 * nrow + top_pad
    fig, axes = plt.subplots(nrow, ncol, figsize=(1.8 * ncol, fig_h), squeeze=False,
                             gridspec_kw={"hspace": 0.55, "wspace": 0.25})
    fig.subplots_adjust(top=1 - top_pad / fig_h)
    for ax in axes.flat:
        ax.axis("off")
    im = None
    ...
    fig.suptitle(title or f"Per-unit stim-window activation (sorted by preferred stim) — "
                 f"{MODELS[model_type]['label']}  [{len(seeds)} seeds]",
                 y=1 - 0.3 * top_pad / fig_h, fontsize=13)
    return fig
